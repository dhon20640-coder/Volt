...
QuestIlha.QuestData = {
    ...
    [2525] = {NameQuest = "TikiQuest2", LevelQuest = 2, Mon = "Isle Champion", CFrameQuest = CFrame.new(-16539.078125, 55.68632888793945, 1051.5738525390625), CFrameMon = CFrame.new(-16933.2129, 93.3503036, 999.450989)},     -- NOVAS QUESTS ADICIONADAS (SUBMERGED)
...
local KM={"Cake Guard","Cookie Crafter","Baking Staff","Head Baker","Cocoa Warrior","Chocolate Bar Battler","Sweet Thief","Candy Rebel","Candy Pirate","Choco Commander"}
local KS=CFrame.new(-2077,38,-12373)
local BonM={"Reborn Skeleton","Living Zombie","Demonic Soul","Posessed Mummy"}
local BS=CFrame.new(-9506,142,5966)
local SubMobs={"Terrorshark","Piranha","Shark","Fish Crew Member"}
local SubNPC=CFrame.new(-5576,-1947,2040)
local ATween;function SAT()if ATween then ATween:Cancel();ATween=nil end end
function TF25(hrp,cf,s)local d=(hrp.Position-cf.Position).Magnitude;hrp.CFrame=CFrame.new(cf.Position.X+25,cf.Position.Y+25,cf.Position.Z+25);ATween=TS:Create(hrp,TweenInfo.new(d/s,Enum.EasingStyle.Linear),{CFrame=cf*CFrame.new(0,30,0)});ATween:Play()end
function TTG(hrp,cf,s)local d=(hrp.Position-cf.Position).Magnitude;ATween=TS:Create(hrp,TweenInfo.new(d/s,Enum.EasingStyle.Linear),{CFrame=cf});ATween:Play()end
function BringMobs(mn)local c=LP.Character;if not c or not c:FindFirstChild("HumanoidRootPart")then return end;local hrp=c.HumanoidRootPart;if not getgenv().BM then return end;for _,m in pairs(WS.Enemies:GetChildren())do if m.Name==mn and m:FindFirstChild("HumanoidRootPart")and m:FindFirstChild("Humanoid")and m.Humanoid.Health>0 then local mr=m.HumanoidRootPart;if IMN(mr)then mr.CFrame=hrp.CFrame*CFrame.new(0,0,getgenv().BM/2);mr.CanCollide=false;mr.Size=Vector3.new(60,60,60)end end end end
function EQ()local c=LP.Character;if not c then return end;local wp=getgenv().WP;if wp and c:FindFirstChild(wp)then local t=c[wp];if not t.Parent then c.Humanoid:EquipTool(t)end end end
local function ISI()local c,hrp=LP.Character,LP.Character and LP.Character:FindFirstChild("HumanoidRootPart");if c and hrp then return hrp.Position.Y<-1800 end;return false end
local function ISM(mn)for _,sm in ipairs(SubMobs)do if mn==sm then return true end end;return false end
...
local function CSS()if GL()<2600 or not World3 then return end;if ISI()then getgenv().SubmergedReady=true;return end;local c,hrp=LP.Character,LP.Character and LP.Character:FindFirstChild("HumanoidRootPart");if not c or not hrp then return end;SAT();TTG(hrp,SubNPC,350);task.wait(1);local ok=pcall(function()RS.Modules.Net:FindFirstChild("RF/SubmarineWorkerSpeak"):InvokeServer("TravelToSubmergedIsland")end);if ok then TS2S();local wt=0;while not getgenv().SubmergedReady and wt<20 do task.wait(0.5);wt=wt+0.5 end;task.wait(2)end end
...
-- ============================================
-- SISTEMA DE TWEEN ISOLADO (SEM CONFLITO)
-- ============================================

local ShopTween = nil
local ShopHeartbeat = nil

local function StopShopTween()
    if ShopTween then 
        ShopTween:Cancel() 
        ShopTween = nil 
    end
    if ShopHeartbeat then 
        ShopHeartbeat:Disconnect() 
        ShopHeartbeat = nil 
    end
end

local function TweenToShop(targetCFrame)
    local char = Plr.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
    
    -- Para apenas os tweens da SHOP (não interfere com Elite Hunter)
    StopShopTween()
    
    local distance = (hrp.Position - targetCFrame.Position).Magnitude
    
    ShopTween = TS:Create(hrp, 
        TweenInfo.new(distance / 250, Enum.EasingStyle.Linear), 
        {CFrame = targetCFrame}
    )
    ShopTween:Play()
    
    -- Anti-tremor isolado APENAS para compras
    ShopHeartbeat = RSvc.Heartbeat:Connect(function()
        if not hrp or not hrp.Parent then 
            StopShopTween() 
            return 
        end
        -- Só aplica velocidade zero se NÃO estiver no Elite Hunter
        if not getgenv().AutoElite then
            hrp.Velocity = Vector3.new(0, 0, 0)
        end
    end)
    
    ShopTween.Completed:Wait()
    StopShopTween()
...
local function BuyFightingStyle(styleName, remoteCommand)
    -- NÃO EXECUTA SE ELITE HUNTER ESTIVER ATIVO
    ...
    local success = TweenToShop(npcPos)
    if success then
        task.wait(0.5)
        pcall(remoteCommand)
        return true
...
-- ============================================
-- SISTEMA DE AUTO BUY EM LOOP (COM PRIORIDADE)
-- ============================================

task.spawn(function()
    while task.wait(5) do
        -- SÓ EXECUTA SE ELITE HUNTER NÃO ESTIVER ATIVO
        if getgenv().AutoElite then 
            continue 
        end
...
print("[VOLT] Módulo Compras - Loaded Successfully!")
-- ============================================
