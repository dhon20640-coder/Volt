...
getgenv().CurrentQuestName,getgenv().CurrentQuestLevel,getgenv().KataIndex,getgenv().BoneIndex,getgenv().AV3,getgenv().GRaceClickAutov4,getgenv().FarmMode,getgenv().LastQuestAttempt,getgenv().GSelectWeapon,getgenv().TweenCompleted,getgenv().SubmergedReady=nil,nil,1,1,false,false,"Farm Level",0,nil,false,false
...
local function HMR(mList,iVar,sCF)local cn=mList[getgenv()[iVar]];if CMC(cn)>0 then local fMob=FMS(cn);if fMob then local h,r=fMob:FindFirstChild("Humanoid"),fMob:FindFirstChild("HumanoidRootPart");if h and r and h.Health>0 then local c,hrp,hum=LP.Character,LP.Character:FindFirstChild("HumanoidRootPart"),LP.Character:FindFirstChild("Humanoid");if c and hrp and hum then PM(c,hrp,hum,fMob,cn)end end end else getgenv()[iVar]=getgenv()[iVar]+1;if getgenv()[iVar]>#mList then getgenv()[iVar]=1 end end;local c,hrp=LP.Character,LP.Character and LP.Character:FindFirstChild("HumanoidRootPart");if c and hrp and CMC(cn)==0 then SAT();TTG(hrp,sCF,250);task.wait(0.5)end end
...
-- Verificação de mundo (Sistema Volt - sempre funciona)
...
-- Função TTG (Tween sem tremedura do Volt)
...
function CheckNearestTeleporter(aI)
    ...
    for r, v in pairs(TableLocations2) do
        if v < min then min, min2 = v, v end
    end
    
    local choose
    for r, v in pairs(TableLocations2) do
        if v <= min then choose = TableLocations[r] end
    end
...
function requestEntrance(aJ)
    ...
    Plr.Character.HumanoidRootPart.CFrame = CFrame.new(oldcframe.X, oldcframe.Y + 50, oldcframe.Z)
    task.wait(0.5)
end
...
function topos(Tween_Pos)
    pcall(function()
        local char = Plr.Character
        if not char or not char:FindFirstChild("Humanoid") or not char:FindFirstChild("HumanoidRootPart") 
           or char.Humanoid.Health <= 0 then return end
...
local IslandsBySea = {
    [1] = {"StartingArea", "Jungle", "Pirate Village", "Desert", "Frozen Village", "MarineFord", "Colosseum", "Sky Island 1", "Sky Island 2", "Sky Island 3", "Prison", "Magma Village", "Under Water Island", "Upper Skylands", "Fountain City"},
    [2] = {"Kingdom of Rose", "Cafe", "Mansion", "Graveyard", "Snow Mountain", "Hot and Cold", "Cursed Ship", "Ice Castle", "Forgotten Island", "Dark Arena", "Green Zone", "Swan Room"},
    [3] = {"Port Town", "Hydra Island", "Great Tree", "Castle On The Sea", "Floating Turtle", "Haunted Castle", "Sea of Treats", "Tiki Outpost"}
}
...
TG_AutoChest:OnChanged(function(v) getgenv().AutoChest = v if v then getgenv().ChestCollected = 0 getgenv().IgnoredChests = {} else SAT() RNP() end end)
...
TG_Island:OnChanged(function(v)
    ...
    if v then
        task.spawn(function()
            ...
            -- Aguardar até chegar no destino
...
TG_ChangeTeam:OnChanged(function(v)
    if v then
        task.spawn(function()
            local currentTeam, selectedTeam = GetCurrentTeam(), getgenv().SelectedTeam
...
-- MÓDULO AUTO FARM MATERIAL + AUTO RAID
...
-- Variáveis Globais Material
getgenv().AMaterial = false
getgenv().SelectedMaterial = nil
local MaterialTweenActive = false

-- Variáveis Globais Raid
getgenv().AutoBuyChip = false
getgenv().Auto_StartRaid = false
getgenv().SelectedRaid = "Flame"
local LastChipBuyTime = 0

-- Ordem de valor das frutas (Menor → Maior) até 1mi
local FruitOrder = {
    "Spin", "Chop", "Spring", "Bomb", "Smoke", "Flame",
    "Ice", "Sand", "Dark", "Light", "Rubber", "Barrier", "Magma"
}

-- Lista de Raids disponíveis
local RaidList = {
    "Flame", "Ice", "Quake", "Light", "Dark", "Spider",
    "Rumble", "Magma", "Buddha", "Sand", "Phoenix", "Dough"
}

-- Função auxiliar para verificar item no Backpack
...
-- Função para ajudar no controle de NoClip
local function SNL(char, enabled)
    if not char then return end
    pcall(function()
...
-- Pega próxima fruta do inventário (sequencial menor → maior)
function GetNextFruitFromInventory()
    local success = false
    pcall(function()
        local fruits = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventory")
        local foundFruits = {}
        
        -- Coleta todas frutas do inventário com índice de valor
        for i, v in pairs(fruits) do
            if type(v) == "table" and v.Type == "Blox Fruit" then
                local fruitValue = 0
                for order, name in ipairs(FruitOrder) do
                ...
                if fruitValue > 0 then
                    table.insert(foundFruits, {name = v.Name, value = fruitValue})
                end
            end
        end
        
        -- Ordena por valor (menor primeiro)
        table.sort(foundFruits, function(a, b) return a.value < b.value end)
        
        -- Pega a MENOR fruta disponível
        for _, fruit in ipairs(foundFruits) do
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StoreFruit", 
                fruit.name, 
                Plr.Backpack:FindFirstChild(fruit.name)
            )
            print("✅ Pegou fruta (valor " .. fruit.value .. "): " .. fruit.name)
            success = true
            return fruit.name
        end
    end)
...
-- Verifica se tem fruta no inventário (Backpack)
function HasFruitInInventory()
    ...
    pcall(function()
        ...
        -- Verifica também no Character (fruta equipada)
...
-- Compra chip com Mystery Scientist
function BuyRaidChip()
    pcall(function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("RaidsNpc", "Select", getgenv().SelectedRaid)
        print("✅ Comprou chip de " .. getgenv().SelectedRaid)
    end)
end

-- ===== UI - Tab Farming (Material) =====
...
-- ===== UI - Tab Raid And Fruit (NOVA ABA) =====
local TabRaid = Win:AddTab({
    Title = "Tab Raid And Fruit",
    Icon = "zap"
})

TabRaid:AddSection("Tab Raid")

local RaidDropdown = TabRaid:AddDropdown("RaidSel", {
    Title = "Select Raid",
    Values = RaidList,
    Default = 1
})

RaidDropdown:OnChanged(function(v)
    getgenv().SelectedRaid = v
    print("Raid selecionada: " .. v)
end)

local TG_BUY_CHIP = TabRaid:AddToggle("AutoBuyChp", {
    Title = "Auto Buy Chip",
    Default = false
})

TG_BUY_CHIP:OnChanged(function(v)
...
local TG_AUTO_RAID = TabRaid:AddToggle("AutoRd", {
    Title = "Auto Start Raid",
    Default = false
})

TG_AUTO_RAID:OnChanged(function(v)
    getgenv().Auto_StartRaid = v
end)
...
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if not getgenv().AutoBuyChip then return end
            
            local currentTime = tick()
            
            -- Cooldown de 1 segundo
            if currentTime - LastChipBuyTime < 1 then return end
            
            -- Verifica se já tem chip no Backpack
            if GetBP("Special Microchip") then return end
            
            -- Tenta pegar fruta do inventário
            if not HasFruitInInventory() then
                GetNextFruitFromInventory()
                task.wait(0.3)
            end
            
            -- Se tem fruta no inventário, compra chip
            if HasFruitInInventory() then
                BuyRaidChip()
                LastChipBuyTime = currentTime
            end
        end)
    end
end)

-- ===== AUTO START RAID (GRAVITY HUB ORIGINAL) =====
task.spawn(function()
    while task.wait(0.1) do
        if not getgenv().Auto_StartRaid then
            continue
        end
        ...
        pcall(function()
            ...
            -- Se já estiver em raid, não faz nada
            ...
            -- Second Sea
            if World2 then
                local Button = WS.Map.CircleIsland.RaidSummon2.Button.Main
                if Button then
            ...
            -- Third Sea
            if World3 then
                local Button = WS.Map["Temple of Time"].RaidSummon2.Button.Main
                if Button then
                    if Button:FindFirstChild("ProximityPrompt") then
                        fireproximityprompt(Button.ProximityPrompt)
                    elseif Button:FindFirstChild("ClickDetector") then
...
-- ===== LOOP PRINCIPAL DO MATERIAL FARM =====
task.spawn(function()
...
-- Sistema de RegisterHit e RegisterAttack para Material
task.spawn(function()
    while task.wait(0.1) do
...
print("✅ Módulo Auto Farm Material + Auto Raid carregado com sucesso!")
